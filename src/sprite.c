...
// - check https://github.com/mruby/mruby/blob/master/mrbgems/mruby-time/src/time.c#L565 and https://dev.to/roryo/storing-c-data-in-an-mruby-class-50k4
...
static mrb_value mrb_cute_sprite_draw(mrb_state* mrb, mrb_value self) {
  mrb_cute_sprite_data_t *data = (CF_Sprite*)mrb_data_get_ptr(mrb, self, &mrb_cute_sprite_data_type);
  if (data == NULL) {
...
static mrb_value mrb_cute_sprite_make_demo_sprite(mrb_state *mrb, mrb_value self) {
  // TODO: https://github.com/mruby/mruby/issues/9#issuecomment-147070792
  mrb_cute_sprite_data_t *data = (mrb_cute_sprite_data_t*)mrb_malloc(mrb, sizeof(mrb_cute_sprite_data_t));
  ...
  *sprite = cf_make_demo_sprite();
  ...
  // TODO: https://github.com/mruby/mruby/blob/master/mrbgems/mruby-time/src/time.c
...
{
  struct RClass* mrb_cute_sprite = mrb_define_class_under(mrb, mrb_cute, "Sprite", mrb->object_class);;

  MRB_SET_INSTANCE_TT(mrb_cute_sprite, MRB_TT_DATA);

  mrb_define_class_method(mrb, mrb_cute_sprite, "make_demo_sprite", mrb_cute_sprite_make_demo_sprite, MRB_ARGS_NONE());

  mrb_define_method(mrb, mrb_cute_sprite, "initialize", mrb_cute_sprite_make_demo_sprite, MRB_ARGS_NONE());
  mrb_define_method(mrb, mrb_cute_sprite, "draw", mrb_cute_sprite_draw, MRB_ARGS_NONE());
  mrb_define_method(mrb, mrb_cute_sprite, "play", mrb_cute_sprite_play, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, mrb_cute_sprite, "playing?", mrb_cute_sprite_is_playing, MRB_ARGS_REQ(1));
  mrb_define_method(mrb, mrb_cute_sprite, "update", mrb_cute_sprite_update, MRB_ARGS_NONE());
}
